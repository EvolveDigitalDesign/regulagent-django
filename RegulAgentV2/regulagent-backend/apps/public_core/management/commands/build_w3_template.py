"""
Django management command: build_w3_template

Stamps AcroForm widgets onto the blank W-3 template, producing an annotated
template that the PDF generator can fill by widget name.

Usage:
    python manage.py build_w3_template
    python manage.py build_w3_template --output /path/to/output.pdf
    python manage.py build_w3_template --verify
    python manage.py build_w3_template --verify --output /path/to/template.pdf
    python manage.py build_w3_template --verify-only
    python manage.py build_w3_template --verify-only --output /path/to/template.pdf
"""
...
class Command(BaseCommand):
    help = "Build the annotated W-3 template with named AcroForm widgets"
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stderr.write(self.style.ERROR(
                f"Failed to import w3_template_builder (is PyMuPDF installed?): {e}"
            ))
            return
        ...
        if options.get("verify") or options.get("verify_only"):
            ...
            self.stdout.write(f"  Page 2 widgets: {info['page_2_widgets']}")
